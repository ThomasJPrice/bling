      ...
      <div className="relative w-full flex justify-center items-center flex-grow">
        <Card className="w-full max-w-md shadow-md m-4">
          <CardHeader>
            ...
            <Link href={`https://www.strava.com/oauth/authorize?client_id=${process.env.STRAVA_CLIENT_ID}&redirect_uri=${process.env.NEXT_PUBLIC_URL}/api/strava/callback&response_type=code&scope=read,read_all,activity:read&approval_prompt=force`} className="!mt-4" target="_blank">
              {/* <Button className='w-full bg-[#FC4C02] hover:bg-[#FC4C02]/90 text-foreground'><FaStrava /> Connect Strava</Button> */}
              <Image src='/strava-connect.png' width={200} height={50} alt="Connect with Strava" className="w-1/2 mx-auto" />
            </Link>
          </CardHeader>

        </Card>

        <div className='absolute -z-10 top-0 left-0 w-full h-full bg-black/20'></div>
        <Image src='/login-image.png' alt='Woman running looking happy' fill className='object-cover -z-20 absolute w-full h-full top-0 left-0' />
      </div>
